...
describe('ProfileStatus component', () => {
  test('status from props should be in the state', () => {
    const component = create(<ProfileStatus status="I am hallow" updateUserStatus={(x)=>x}/>);
    const instance = component.getInstance();
    expect(instance?.props.status).toBe('I am hallow');
  });
  ...
  test('after creation <span> should contains correct status', async () => {
    const component = create(<ProfileStatus status="it-kamasutra" updateUserStatus={(x)=>x}/>);
    const root = component.root;
